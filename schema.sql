-- Calendar Aggregator Schema and Collections table for persistent storage
-- This schema isolates calendar aggregator resources from other applications
-- Replaces the in-memory globalThis.calendarCollections storage

-- Create the calendar_aggregator schema
CREATE SCHEMA IF NOT EXISTS calendar_aggregator;

-- Grant usage permissions to anon and authenticated roles
GRANT USAGE ON SCHEMA calendar_aggregator TO anon, authenticated;

-- Create collections table in the calendar_aggregator schema
